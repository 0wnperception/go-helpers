//nolint:dupl,mnd,err113,exhaustive,wsl,lll,goconst,gocritic,whitespace,errorlint,revive,funlen,gocognit,gocyclo,gochecknoglobals,unparam,godot
package gen
...
func (g *Generator) genTypeDecoderNoCheck(t reflect.Type, out string, tags fieldTags, indent int) error {
	...
	case reflect.Slice:
		...
		if elem.Kind() == reflect.Uint8 && elem.Name() == "uint8" {
			...
			capacity := 1
			if elem.Size() > 0 {
				capacity = minSliceBytes / int(elem.Size()) //nolint:gosec
			}
			...
			fmt.Fprintln(g.out, ws+"      "+out+" = make("+g.getType(t)+", 0, "+strconv.Itoa(capacity)+")")
			fmt.Fprintln(g.out, ws+"    } else {")
	...
	case reflect.Array:
		...
		if elem.Kind() == reflect.Uint8 && elem.Name() == "uint8" {
			...
			fmt.Fprintln(g.out, ws+"  for !in.IsDelim(']') {")
			fmt.Fprintln(g.out, ws+"    if "+iterVar+" < "+strconv.Itoa(length)+" {")
	...
	case reflect.Interface:
		if t.NumMethod() != 0 {
			if g.interfaceIsEasyjsonUnmarshaller(t) {
				...
				return fmt.Errorf("interface type %v not supported: only any and easyjson/json Unmarshaler are allowed", t)
			}
		} else {
...
func (g *Generator) genRequiredFieldSet(_ reflect.Type, f reflect.StructField) {
	if tags := parseFieldTags(f); !tags.required {
		return
	}
...
func mergeStructFields(fields1, fields2 []reflect.StructField) []reflect.StructField {
	//nolint: prealloc
...
func (g *Generator) genStructDecoder(t reflect.Type) error {
	...
	if err != nil {
		return fmt.Errorf("cannot generate decoder for %v: %w", t, err)
	}
