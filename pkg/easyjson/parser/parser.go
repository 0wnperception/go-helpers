//nolint:wsl,mnd,gocritic,revive,nakedret
package parser
...
func (v *visitor) Visit(n ast.Node) ast.Visitor {
	switch n := n.(type) {
	case *ast.Package: //nolint:staticcheck
		return v
	case *ast.File:
...
func (p *Parser) Parse(fname string, isDir bool) error {
	...
	if isDir {
		packages, e := parser.ParseDir(fset, fname, excludeTestFiles, parser.ParseComments)
		if e != nil {
			return e
		}
		...
		f, e := parser.ParseFile(fset, fname, nil, parser.ParseComments)
		if e != nil {
			return e
		}
